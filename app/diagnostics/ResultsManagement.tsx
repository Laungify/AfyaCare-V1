                ...
                {testCode.includes('US-') ? (
                  <div className="space-y-2">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{testData.findings}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-700">Impression:</p>
                      <p className="text-sm text-gray-600">{testData.impression}</p>
                    </div>
                  </div>
                ) : testCode === 'ECG' ? (
                  <div>
                  ...
                  <div className="space-y-2">
                    <p className="text-sm"><span className="font-medium">Result:</span> {testData.result}</p>
                    {testData.species && <p className="text-sm"><span className="font-medium">Species:</span> {testData.species}</p>}
                  </div>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {Object.entries(testData).map(([param, data]) => (
                          <tr key={param} className="border-t">
                            <td className="p-2 font-medium">{param}</td>
                            <td className={`p-2 font-semibold ${
                              data.status?.includes('critical') ? 'text-red-600' :
                            ...
                            <td className="p-2">
                              <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                data.status?.includes('critical') ? 'bg-red-100 text-red-700' :
            ...
            <div className="flex items-start justify-between">
              <div className="flex items-start space-x-4 flex-1">
                <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${
                  result.category === 'Laboratory' ? 'bg-blue-100' :
                  result.category === 'Imaging' ? 'bg-purple-100' : 'bg-green-100'
                }`}>
                  <i className={`${getCategoryIcon(result.category)} text-xl ${
                    result.category === 'Laboratory' ? 'text-blue-600' :
